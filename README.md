...
- `supabase.auth.signUp()`의 `data.session`: Confirm email이 꺼져있으면 가입과 동시에 세션이 채워져서 오고, 켜져있으면 `null`로 옴 — 이 값으로 "바로 로그인" vs "메일 확인 필요"를 분기할 수 있음

### 막힌 점 / 해결
- 회원가입 시도 시 브라우저에 "Failed to fetch"만 뜸 → 개발자도구 네트워크 탭으로 실제 요청을 보니 Supabase 쪽에서 503(Service Unavailable) 응답 → Supabase 무료 프로젝트가 일정 기간 미사용으로 자동 일시정지(paused)된 것이 원인 → 대시보드에서 Resume project로 재개해 해결. **교훈: 무료 플랜은 며칠 방치하면 자동으로 꺼지니, 갑자기 API가 안 되면 제일 먼저 프로젝트 상태부터 확인할 것**
- 프로젝트 재개 후에도 회원가입 시 확인 이메일이 오지 않음 → Supabase 기본 이메일 발송이 무료 플랜에서 시간당 발송 제한이 있어 매우 불안정 → 개발 단계에서는 Authentication > Sign In/Providers의 "Confirm email"을 꺼서 가입 즉시 로그인되도록 임시 처리 (실서비스 출시 시 별도 이메일 발급 서비스 연동 필요)
- Confirm email을 껐는데도 회원가입 성공 시 화면에 계속 "가입 확인 이메일을 보냈습니다"가 뜸 → 코드가 `signUp()`의 `data.session` 값을 확인 안 하고 무조건 그 메시지를 보여주고 있었음 → `data.session` 유무로 분기해서 세션이 있으면 바로 홈으로 이동하도록 수정

### 결과
- 로드맵 14주차 목표(로그인 기능) 완전히 마무리 — 회원가입, 로그인, 로그아웃, 로그인 상태 UI 반영까지 전부 작동 확인
- Supabase 프로젝트 일시정지 문제를 실전에서 직접 진단하고 해결 — 네트워크 탭으로 원인 추적하는 디버깅 경험 확보
